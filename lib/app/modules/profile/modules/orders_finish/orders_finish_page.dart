...
class _OrdersFinishPageState extends State<OrdersFinishPage> {
  final ValueNotifier<List<OrderModel>> _filteredOrders = ValueNotifier<List<OrderModel>>([]);
  String searchQuery = '';
  ...
  Future<void> _abrirDetalhes(OrderModel order) async {
    ...
    final details = await repo.getOrderDetails(pedido: order.pedido, implemented: true);
    ...
    return Scaffold(
      body: Stack(children: [
        const Positioned.fill(
          child: Image(image: AssetImage('assets/bg-login.jpg'), fit: BoxFit.cover),
        ),
        Column(children: [
          Padding(
                ...
                child: DropdownButtonFormField<int>(
                  ...
                  items: List.generate(12, (i) {
                    ...
                    return DropdownMenuItem(
                      value: month,
                      child: Text(
                        DateFormat.MMMM('pt_BR').format(DateTime(0, month)).toUpperCase(),
                        style: const TextStyle(color: Colors.black),
                      ),
                    );
                  }),
              ...
              Expanded(
                child: DropdownButtonFormField<int>(
                  ...
                  items: years
                    ...
                    value: y,
                    child: Text(y.toString(), style: const TextStyle(color: Colors.black)),
                  ))
                      .toList(),
                  onChanged: (val) async {
                    selectedYear = val!;
          ...
          Expanded(
            child: BlocConsumer<OrderBlocCubit, OrderBlocState>(
              listener: (context, state) async {
                if (state.status == OrderStateStatus.success && state.orderModel != null) {
                  await _atualizarFiltroAsync(state.orderModel!);
                }
              },
              builder: (context, state) {
                if (state.status == OrderStateStatus.loading || _isFiltering) {
